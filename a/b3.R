...
for(ii in 1:n){
  for(jj in ii:n){
    for(ll in 1:Mm){
      idx[ii,jj,ll] = kk
      idx[jj,ii,ll] = kk
      coef_name[kk] = paste0("a",ii,jj,"_",phi_name[ll])
      kk=kk+1
    }
  }
}
if(Mp>0){
  for(ll in 1:Mp){
    jdx[ll] = kk
    coef_name[kk] = paste0("b","_",phi_name[ll+1])
    kk=kk+1
  }
}
vv=1
...
#軌道に沿った基底関数の計算
psi = array(0,dim=c(Mm,Tx))
psid = array(0,dim=c(n,Mm,Tx))
for(ll in 1:Mm){
  for(tt in 1:Tx){
    psi[ll,tt] = phi[[ll]](X[tt,1],X[tt,2])
    for(kk in 1:n){
      psid[kk,ll,tt] = phid[[kk]][[ll]](X[tt,1],X[tt,2])
    }
  }
}
  ...
  for(kk in 1:n){
    for(ii in 1:n){
      for(ll in 1:Mm){
        uu = idx[kk,ii,ll]
        vv = tdx[tt,kk]
        # S = Xdd[tt,ii]*phi[[ll]](X[tt,1],X[tt,2])
        S = Xdd[tt,ii]*psi[ll,tt]
        P[vv,uu] = P[vv,uu] + S
        
        for(jj in 1:n){
          uu = idx[ii,kk,ll]
          # S = 0.5*Xd[tt,ii]*Xd[tt,jj]*phid[[jj]][[ll]](X[tt,1],X[tt,2])
          S = 0.5*Xd[tt,ii]*Xd[tt,jj]*psid[jj,ll,tt]
          P[vv,uu] = P[vv,uu] + S
          
          uu = idx[kk,jj,ll]
          # S = 0.5*Xd[tt,ii]*Xd[tt,jj]*phid[[ii]][[ll]](X[tt,1],X[tt,2])
          S = 0.5*Xd[tt,ii]*Xd[tt,jj]*psid[ii,ll,tt]
          P[vv,uu] = P[vv,uu] + S
          
          uu = idx[ii,jj,ll]
          # S = -0.5*Xd[tt,ii]*Xd[tt,jj]*phid[[kk]][[ll]](X[tt,1],X[tt,2])
          S = -0.5*Xd[tt,ii]*Xd[tt,jj]*psid[kk,ll,tt]
          P[vv,uu] = P[vv,uu] + S
...
for(tt in 1:(Tx-1)){
  for(ii in 1:n){
    for(jj in 1:n){
      for(ll in 1:Mm){
        uu = idx[ii,jj,ll]
        # S = phi[[ll]](X[tt,1],X[tt,2])*Xd[tt,ii]*Xd[tt,jj]/2 - phi[[ll]](X[tt+1,1],X[tt+1,2])*Xd[tt+1,ii]*Xd[tt+1,jj]/2
        S = psi[ll,tt]*Xd[tt,ii]*Xd[tt,jj]/2 - psi[ll,tt+1]*Xd[tt+1,ii]*Xd[tt+1,jj]/2
  ...
  if(Mp>0){
    for(ll in 1:Mp){
      uu = jdx[ll]
      # S = phi[[ll+1]](X[tt,1],X[tt,2]) - phi[[ll+1]](X[tt+1,1],X[tt+1,2])
      S = psi[ll+1,tt] - psi[ll+1,tt+1]
      PHc[tt,uu] = PHc[tt,uu] + S
    }
  }
}
  ...
  if(tt==1){
    for(ii in 1:n){
      for(jj in 1:n){
        for(ll in 1:Mm){
          uu = idx[ii,jj,ll]
          # S = phi[[ll]](X[tt,1],X[tt,2])*Xd[tt,ii]*Xd[tt,jj]/2 
          S = psi[ll,tt]*Xd[tt,ii]*Xd[tt,jj]/2 
...
# ScQ = Q%*%iPtP%*%t(Q)
# c_10 = iPtP%*%t(Q)%*%solve(ScQ,b,tol=0) # Schur complement T C

c_11 = solve(t(M)%*%M,t(M)%*%u,tol = 0) # penalty T C
# c_11b = solve(t(rbind(P,PH1))%*%rbind(P,PH1),t(rbind(P,PH1))%*%rbind(v,vH1),tol = 0) # penalty H
...
dn();dC %>% ggplot_bw(aes(y=basis,x=coef/scale/pol,shape=st,colour = st,group = paste(method,st)))+
  geom_point(alpha=0.5)+geom_line(aes(linetype=st))+facet_wrap(~method,nrow=1)+scale_y_discrete(limits = coef_name)
...
dn();dC %>% separate(basis,into=c("aij","phi"),sep="_",remove = FALSE) %>% filter(aij=="a11") %>% 
  ...
  geom_point(alpha=0.5)+scale_y_discrete(drop=FALSE)+
  facet_grid(aij~method)
dn();dC %>% separate(basis,into=c("aij","phi"),sep="_",remove = FALSE) %>% filter(aij=="a12") %>% 
  ggplot_bw(aes(y=basis,x=coef/scale/pol,shape=st,colour = st,group = paste(method,st)))+
  geom_point(alpha=0.5)+scale_y_discrete(drop=FALSE)+
  facet_grid(aij~method)
dn();dC %>% separate(basis,into=c("aij","phi"),sep="_",remove = FALSE) %>% filter(aij=="a22",st=="T") %>% 
  ggplot_bw(aes(y=basis,x=coef/scale/pol,shape=st,colour = st,group = paste(method,st)))+
  geom_point(alpha=0.5)+scale_y_discrete(drop=FALSE)+
  facet_grid(aij~method,scales="free_x")
...
dn();dC %>% separate(basis,into=c("aij","phi"),sep="_",remove = FALSE) %>% filter(aij=="b",st=="T") %>% 
  ggplot_bw(aes(x=basis,y=coef/scale/pol,shape=st,colour = st,group = paste(method,st)))+
  geom_point(alpha=0.5)+geom_hline(yintercept = 0,linetype = "dotted")+
  facet_grid(method~aij,scales="free_y")+theme(axis.text.x = element_text(angle = -45, hjust = 0))
